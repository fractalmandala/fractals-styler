...
| Semantic inconsistency    | `xcenter`/`ycenter` modifiers mean different things on `.box` vs `.row`                                                   |
...
#### Still static (hard-coded px)

- `--text-xs` (10px), `--text-sm` (12px), `--text-md` (14px) — 3 static type sizes
- `--space-1` (4px) — 1 static space size
...
#### Button/control spacing with tokens

In `_buttonslinks.sass`:

- `gap: 8px` becomes `gap: var(--space-2xs)`
- `padding: 8px 16px` becomes `padding: var(--space-2xs) var(--space-xs)`
...
| `_buttonslinks.sass` | Replace static `8px`/`16px` with space tokens                                      |
...
| 6   | Make all type tokens fluid (remove static px from xs/sm/md) | Task 2 | Low     |
...
| 20  | Make layout tokens fluid (header/footer/shell)          | Task 2 | Low     |
| 21  | Replace static px in buttons/controls with space tokens | Task 2 | Trivial |
| 22  | Align `bp-xl` breakpoint to 1240px                      | Task 2 | Trivial |

### Tier 6 — Code hygiene

| #   | Action                                                              | Source | Effort  |
| --- | ------------------------------------------------------------------- | ------ | ------- |
| 23  | Resolve duplicate `.blank` definition                               | Task 1 | Trivial |
| 24  | Rename `.icon-button` to follow grammar (use `data-variant`)        | Task 1 | Low     |
| 25  | Extend global margin reset to `ul, ol, figure, blockquote`          | Task 3 | Trivial |
| 26  | Extend `box-sizing` to `*::before, *::after`                        | Task 3 | Trivial |
| 27  | Add responsive media global (`img, video, svg { max-width: 100% }`) | Task 3 | Trivial |
| 28  | Fix `.prim` reference typo in `02-principles.md`                    | Task 1 | Trivial |
...
- **Static values to make fluid:** 4 type tokens + 1 space token + 6 layout/component values
- **New compositions proposed:** 7 with complete SASS code
- **Total action items:** 28 prioritized across 6 tiers
