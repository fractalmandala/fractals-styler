...
- **Static SASS** lives in `templates/` — `tokens → colors → typography → globals → primitives → compositions → buttonslinks`, all imported via `index.sass` in cascade order.
- **Flat tokens**: 12 direct-value tokens (3 backgrounds, 3 text shades, 3 border shades, 3 brand colors) plus a small derived set via `color-mix`. No primitive scale indirection. Mode switch via `[data-mode="light|dark"]` + `prefers-color-scheme` auto, with `[data-theme]` reserved for named themes (`[data-theme="light|dark"]` and `.light`/`.dark` remain as deprecated aliases). See [09-mode-and-theme.md](09-mode-and-theme.md).
- **Type scale** matches your taste exactly: `--text-xs/sm/md/lg/xl/2xl/3xl/4xl` → `.text-*` classes that just `font-size: var(--text-*)`.
...
1. Adds semantic text primitives (`.body-std`, `.page-title`, `.eyebrow`) so you stop retyping something like `.text-sm .lh15 .prim` everywhere.
...
7. Keeps arbitrary JIT values available for exploratory work without making them the public design vocabulary.
8. **Flattens the color token system** — removes the `--slate-*` primitive scale, drops `--bg-subtle`/`--bg-overlay`/`--border-focus`, and renames `--bg-app`→`--bg`. The canonical primary text token is `--text-primary`.
...
| role       | `xs`, `sm`, `md`, `lg`, `xl`, `2xl`, `3xl`, `4xl`, `surface`, `raised`, `subtle`, `primary`, `brand` |
| variant    | `top`, `bot`, `left`, `right`, `center`, `between`, `evenly`, `wrap`, `cols-2`                       |
| breakpoint | `sm` (640+), `md` (768+), `lg` (1024+), `xl` (1240+)                                                 |
