...
export function generateSass(node: BuilderNode, depth = 0): string {
	...
	if (node.width === '100%') out += `${indent}\twidth: 100%\n`;
		...
		const val =
			node.widthVal.includes('px') || node.widthVal.includes('rem')
				? node.widthVal
				: `${node.widthVal}px`;
		...
		if (node.minWVal) {
			const minV =
				node.minWVal.includes('px') || node.minWVal.includes('rem')
					? node.minWVal
					: `${node.minWVal}px`;
		...
		if (node.maxWVal) {
			const maxV =
				node.maxWVal.includes('px') || node.maxWVal.includes('rem')
					? node.maxWVal
					: `${node.maxWVal}px`;
	...
	if (node.height === '100%') out += `${indent}\theight: 100%\n`;
		...
		const val =
			node.heightVal.includes('px') || node.heightVal.includes('rem')
				? node.heightVal
				: `${node.heightVal}px`;
		...
		if (node.minHVal) {
			const minV =
				node.minHVal.includes('px') || node.minHVal.includes('rem')
					? node.minHVal
					: `${node.minHVal}px`;
		...
		if (node.maxHVal) {
			const maxV =
				node.maxHVal.includes('px') || node.maxHVal.includes('rem')
					? node.maxHVal
					: `${node.maxHVal}px`;
	...
	if (node.textAlign && node.textAlign !== 'left')
		out += `${indent}\ttext-align: ${node.textAlign}\n`;
...
export function generateSvelteComponent(targetNode: BuilderNode): {
	svelteCode: string;
	sassCode: string;
} {
