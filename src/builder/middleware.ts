	...
	return async (req: IncomingMessage, res: ServerResponse, next: () => void) => {
		...
		if (url.startsWith('/__styler_builder')) {
			const prototypePath = path.resolve(
				rootPath,
				'node_modules/fractals-styler/dev/prototype.html'
			);
			const fallbackPath = path.resolve(
				rootPath,
				'packages/fractals-styler/dev/prototype.html'
			);
			...
			req.on('end', () => {
				try {
					...
					const sassFilePath = path.join(
						targetDir,
						`${componentName.toLowerCase()}.sass`
					);
					...
					res.statusCode = 500;
					res.end(
						JSON.stringify({ error: err.message || 'Failed to save component files' })
					);
				}
			});
			return;
