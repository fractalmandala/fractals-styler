...
<div bind:this={host} class="styler-preview">
	<header class="sp-head">
		...
		<div class="sp-tools">
			...
			<button class="sp-btn sp-theme" onclick={toggleTheme}
	...
	<section class="sp-section">
		...
		{#each TEXT_SAMPLES as sample}
			<div class="sp-row">
				...
				<span
					class="sp-sample"
					style={`color: var(${sample.var})` +
						(sample.inverse ? '; background: var(--theme)' : '')}
					>{sample.label} Text</span
				>
			</div>
		{/each}
	</section>
	...
	<section class="sp-section">
		...
		<div class="sp-row">
			...
			<button class="button" data-variant="quiet">Quiet</button>
			<button class="blank">Blank</button>
		</div>
	</section>

	<footer class="sp-foot">
		Overrides stored to <code>{STORAGE_KEY}</code> in localStorage — <code>Reset</code> clears
		them.
